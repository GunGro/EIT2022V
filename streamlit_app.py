...
with col2:
        ...
        st.write("Each input in the calculator is given an individual score that measures how important it is for calculating risk, on average. Inputs with higher scores have a larger effect on the final percentage. The graph below shows that the inputs ‘Number of Credits Passed’ and ‘Same municipality as Parents’ have the highest importance scores; they are the most important indicators that show a person's risk of committing fraud.")
        ...
        st.write("In order to understand what direction your inputs affect your risk estimate we need an effect plot. This is because an effect plot shows how much, and in which direction, the different values of each input on average affects the risk assessment.")
        st.write("An example of one such effect plot of passed university credits is provided below. This shows how much and in which direction the different values of the passed university credits on average affect the risk assessment. Zero signifies no effect. As you see in the plot, when students have passed 0-180 credits the bar is above zero. This means that having passed few credits signifies that students are more likely to commit fraud. Similarly, when students have passed 300+ credits, the bar is below zero, which means that students having passed many credits are less likely to commit fraud.")
        st.image("./uni_cred3.png")
