...
class DummyModel:
    base_line_odds = 0.50/0.50 # procent to be positive divided by percent to be negative
    ...
    def create_effects_image(self, state_dict = {}):
        fig, ax = plt.subplots(figsize = (14,7),  nrows = 1, ncols=1, sharey=True)
        pd.DataFrame(self.uni_cred).plot.bar("x", "y", ax = ax, label = "Percent Odds Increase", title = "Univercity Credits", xlabel = 'Credits', fontsize = 12, color ="#410464")
        ax.set_facecolor("#F4F4F4")
        ax.axhline(y = 0, color = "black")
        fig.savefig('effect_plot.png', format = 'png', facecolor="#F4F4F4")
